...
```{r}
operations2 = operations%>%filter(surgery_category == "Total hysterectomy" | surgery_category == "Bilateral salpingo_oopherectomy" | surgery_category == "Bilateral oopherectomy")
operations2$opdate = as.Date(operations2$opdate)
operations2 <- operations2 %>%
  filter(!is.na(opdate)) %>%
  group_by(eid)%>%
  summarize(date_op1 = min(opdate))
```
...
D3_cens = run_stitch_cens(b_o_cancers, brca1, pheno2)
```
...
D5 = run_stitch(o_cancers, brca1, pheno5)
D5_cens = run_stitch_cens(o_cancers, brca1, pheno5)
```
...
survival_plot(survres5_cens, D5_cens, "Time to Ovarian Cancer", "event", 40, .6)
```
