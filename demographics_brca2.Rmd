...
```{r}
library(readxl)
demo_table = read_excel('/Users/orszagl2/Documents/Genetics/demographs_brca2.xlsx')
demo_table = demo_table[c(1:4, 7:15), 1:7]
demo_table$`p (UKBB)` = ifelse(demo_table$`p (UKBB)`<0.01, formatC(demo_table$`p (UKBB)`, format = "e", digits = 3), demo_table$`p (UKBB)`)
demo_table$`p (GHS)` = ifelse(demo_table$`p (GHS)`<0.01, formatC(demo_table$`p (GHS)`, format = "e", digits = 3), demo_table$`p (GHS)`)
```
