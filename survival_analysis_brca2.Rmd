...
coxph(Surv(AGEATSTART,TE, res) ~ gr_combo, data=D1)
...
```{r cox plot cancer}
survival_plot_fam(survres1, D1, "Time To All Cancer", "event", 20, .8)
...
```{r}
D3 = run_stitch(b_o_cancers, brca2, pheno2)
```
...
survival_plot_fam(survres3, D3, "Time to Breast/Ovarian Cancer", "event", 20, .6)
...
```{r}
D4 = run_stitch(b_cancers, brca2, pheno2)
```
...
```{r cox plot breast}
survival_plot_fam(survres4, D4, "Time to Breast Cancer", "event", 20, .6)
...
D5 = run_stitch(o_cancers, brca2, pheno2)
```
...
coxph(Surv(AGEATSTART,TE, res) ~ GR, data=D5)
...
```{r cox plot ovarian}
survival_plot_fam(survres5, D5, "Time to Ovarian Cancer", "event", 40, .4)
survival_plot(survres5_nofam, D5, "Time to Ovarian Cancer", "event", 40, .4)
```
